...
function registerExternalSelectionApi(ctx) {
  ...
  store[key] = {
    getState: () => {
      ...
      const iecVerticesRawSet = st.methodIecCasesRawVertices instanceof Set ? st.methodIecCasesRawVertices : iecRawSet;
      const iecBoundaryRawSet = st.methodIecCasesRawBoundary instanceof Set ? st.methodIecCasesRawBoundary : iecRawSet;
      ...
      const iecVerticesVisible = countVisible(iecVerticesRawSet);
    ...
    setShowOnlySelected: (flag) => {
      ...
      applyStateToPlots();
      try {
        if (typeof notifySelectionStateChanged === "function") notifySelectionStateChanged(nextCtx);
      } catch (e) {}
      return true;
    },
