...
- `preselection_payload` (object): server-precomputed Energinet/IEC candidate data per base frequency (`50`/`60`)
  - IEC payload supports both envelope modes:
    - `iec_modes.vertices`
    - `iec_modes.boundary` (includes collinear boundary points)
...
4. Restyles line plots (`x/r/xr`) without rerun:
   ...
   - base-frequency scaling (`x = frequency/f_base`) is server-managed in Streamlit reruns
...
14. Maintains source-aware selection model:
    ...
    - method toggles, method-specific `Top N`, IEC boundary mode, and Energinet thresholds are applied client-side without rerun.
