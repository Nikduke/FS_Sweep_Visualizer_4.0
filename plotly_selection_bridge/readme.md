...
- `preselection_payload` (object): server-precomputed Energinet/IEC candidate data
  - compact format (`format=compact_v1`) is supported for lower payload size:
    - `case_ids` + array metrics (`energinet.z2/z3/z4/...`)
    - IEC modes encoded with `case_idx` + `vertex_orders`
  - legacy dict-per-case payload is still accepted as fallback
