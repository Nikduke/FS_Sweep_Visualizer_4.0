...
function notifySelectionStateChanged(ctx) {
  if (!ctx) return;
  try {
    const root = rootWindow();
    ...
    root.dispatchEvent(
      new EventCtor("fsCaseUiStateChanged", {
        detail: { stateKey, version },
      }),
    );
  } catch (e) {}
}
...
function createDefaultState(partOptions, colorByOptions, colorByDefault, showOnlyDefault, baseFreqOptions, baseFreqDefault) {
  ...
  return {
    ...
    methodIecCasesRawVertices: new Set(),
    methodIecCasesRawBoundary: new Set(),
...
function sanitizeState(state, casesMeta, partOptions, colorByOptions, colorByDefault, baseFreqOptions, baseFreqDefault) {
  ...
  next.methodIecCasesRawVertices = toKnownCaseSet(next.methodIecCasesRawVertices, knownCaseIds);
  next.methodIecCasesRawBoundary = toKnownCaseSet(next.methodIecCasesRawBoundary, knownCaseIds);
...
function ensureContext() {
  ...
  if (prevSelReset !== selectionResetToken) {
    ...
    state.methodIecCasesRawVertices = new Set();
    state.methodIecCasesRawBoundary = new Set();
...
function computeEnerginetCandidates(ctx) {
  ...
  for (const item of ctx.casesMeta) {
    ...
    if (row && typeof row === "object") {
      z2 = Number(row.zmax_band_2);
      z3 = Number(row.zmax_band_3);
      z4 = Number(row.zmax_band_4);
    } else if (hasCompactEnerginet) {
...
function getIecModeData(baseData, modeKey) {
  ...
  return iecModes && iecModes[modeKey] && typeof iecModes[modeKey] === "object"
    ? iecModes[modeKey]
    : baseData;
}
...
function computeIecCandidatesForMode(ctx, modeKey) {
  ...
  const baseData = getPreselectionForBase(ctx);
  if (!baseData) return out;
  const modeData = getIecModeData(baseData, String(modeKey || "vertices"));
  const ranked = [];
  ...
  if (compactIdx.length > 0 && baseCaseIds.length > 0) {
    const seen = new Set();
...
function recomputeSelectedCases(ctx) {
  ...
  const modeKey = Boolean(st.methodIecIncludeCollinear) ? "boundary" : "vertices";
  const cacheKey = [
  ...
  let rawIecVertices = st.methodIecCasesRawVertices instanceof Set ? st.methodIecCasesRawVertices : new Set();
  let rawIecBoundary = st.methodIecCasesRawBoundary instanceof Set ? st.methodIecCasesRawBoundary : new Set();
  const needRecompute =
    String(st.__methodRawCacheKey || "") !== cacheKey
    || !(st.methodEnerginetCasesRaw instanceof Set)
    || !(st.methodIecCasesRawVertices instanceof Set)
    || !(st.methodIecCasesRawBoundary instanceof Set);
  if (needRecompute) {
    rawEnerginet = computeEnerginetCandidates(ctx);
    rawIecVertices = computeIecCandidatesForMode(ctx, "vertices");
    rawIecBoundary = computeIecCandidatesForMode(ctx, "boundary");
    st.methodEnerginetCasesRaw = rawEnerginet;
    st.methodIecCasesRawVertices = rawIecVertices;
    st.methodIecCasesRawBoundary = rawIecBoundary;
    st.__methodRawCacheKey = cacheKey;
  }
  const rawIec = modeKey === "boundary" ? rawIecBoundary : rawIecVertices;
...
function getRxSingleTracePayload(gd) {
  try {
    ...
    if (!freqValues.length || xByStep.length !== freqValues.length || yByStep.length !== freqValues.length) return null;
    return {
      freqValues,
      xByStep,
      yByStep,
...
function scatterSliderEventIndex(evt, gd) {
  ...
  try {
    ...
    if (step && step.value != null) idx = Number(step.value);
    if (!Number.isFinite(idx) && step && Array.isArray(step.args) && step.args.length > 0) {
...
function applyScatterFrequencyIndex(ctx, gd, nextIndex) {
  if (!gd) return false;
  ...
  const win = gd.ownerDocument && gd.ownerDocument.defaultView ? gd.ownerDocument.defaultView : null;
  const Plotly = win && win.Plotly ? win.Plotly : null;
  if (!Plotly || !Plotly.restyle || !Plotly.relayout) return false;
  ...
  const current = Number.isFinite(currentRaw) ? currentRaw : Number(readScatterSliderActiveIndex(gd));
  if (Math.floor(current) === idx) return true;

  const freqHz = Number(payload.freqValues[idx]);
  const seqLabel = String(payload.seqLabel || "");
  const titleText = Number.isFinite(freqHz)
    ? `R vs X at f ~ ${freqHz.toFixed(1)} Hz${seqLabel ? ` (${seqLabel})` : ""}`
    : "";

  const relayoutPayload = { "sliders[0].active": idx };
  if (titleText) relayoutPayload["title.text"] = titleText;
  ...
  try {
    ...
    Plotly.restyle(
      gd,
      {
        x: [payload.xByStep[idx]],
        y: [payload.yByStep[idx]],
      },
      [pointTraceIndex],
    );
    Plotly.relayout(gd, relayoutPayload);
    gd.__fsRxSingleActiveIndex = idx;
    if (ctx) {
      const allowed = computeAllowedSet(ctx);
      applyScatterStyles(ctx, gd, allowed, false);
    }
    return true;
  } catch (e) {
    return false;
...
function bindScatterHandlers(ctx, gd, plotIndex) {
  ...
  const sliderHandler = (evt) => {
    ...
    if (Number.isFinite(idx) && applyScatterFrequencyIndex(nextCtx, gd, idx)) return;
    const allowed = computeAllowedSet(nextCtx);
    applyScatterStyles(nextCtx, gd, allowed, false);
  };
  ...
  try {
    ...
    gd.__fsCaseUiAnimated = animatedHandler;
    gd.__fsCaseUiSlider = sliderHandler;
  } catch (e) {}

  gd.on("plotly_click", clickHandler);
  gd.on("plotly_animated", animatedHandler);
