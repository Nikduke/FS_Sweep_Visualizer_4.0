...
function clearSelectionSyncBindings() {
  ...
  try {
    ...
    if (selectionStateListener && rootWin && typeof rootWin.removeEventListener === "function") {
      rootWin.removeEventListener("fsCaseUiStateChanged", selectionStateListener);
    }
  } catch (e) {}
...
function installSelectionSync(stateKey) {
  ...
  try {
    ...
    if (rootWin && typeof rootWin.addEventListener === "function") {
      rootWin.addEventListener("fsCaseUiStateChanged", selectionStateListener);
    }
  } catch (e) {}
  try {
...
function syncSelectionControls(stateKey, force) {
  try {
    ...
    const iecCollinearLbl = document.getElementById("rx-iec-collinear-label");
    ...
    if (iecCollinearLbl) {
      const extra = Math.max(0, Number(st.iecCollinearAdditionalCount || 0));
      iecCollinearLbl.textContent = `Include collinear boundary (+${extra})`;
    }
    if (t2) {
    ...
    <div id="rx-step-root">
      ...
      <div class="rx-row">
        ...
        <label class="rx-method-check">
          ...
          <span id="rx-iec-collinear-label">Include collinear boundary (+0)</span>
        </label>
      </div>
