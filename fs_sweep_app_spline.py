...
def _base_switch_tracker_key(data_id: str, seq_label: str, location_value: str) -> str:
    return f"base_switch_tracker:{data_id}:{seq_label}:{location_value}"
...
def _prune_chart_scoped_session_state(
    ...
    current_base_tracker_key = _base_switch_tracker_key(did, seq, loc)
    ...
    for key in list(st.session_state.keys()):
        ...
        if k.startswith(f"preselection_payload:{did}:") and k != current_preselection_key:
            st.session_state.pop(k, None)
            continue
        if k.startswith(f"base_switch_tracker:{did}:") and k != current_base_tracker_key:
            st.session_state.pop(k, None)
            continue
...
def _maybe_evict_caches_on_base_switch(
    data_id: str,
    seq_label: str,
    selected_location: str,
    f_base: float,
    ...
    tracker_key = _base_switch_tracker_key(did, seq, loc)
    prev_raw = st.session_state.get(tracker_key)
    prev_base: Optional[int] = None
    if prev_raw is not None:
        try:
            prev_base = int(round(float(prev_raw)))
        except Exception:
            prev_base = None
    cur_base = int(round(float(f_base)))

    if prev_base is not None and prev_base != cur_base:
        _evict_sequence_render_caches(did, seq)
        st.session_state.pop(_preselection_cache_key(did, seq, loc), None)

    st.session_state[tracker_key] = int(cur_base)
...
def main():
    ...
    _maybe_evict_caches_on_base_switch(
        data_id=str(data_id),
        seq_label=str(seq_label),
        selected_location=str(selected_location),
        f_base=float(f_base),
