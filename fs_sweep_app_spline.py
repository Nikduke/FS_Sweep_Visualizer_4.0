...
def make_spline_traces(
    ...
    for case in cases:
        ...
        mode = "lines" if enable_spline else "lines+markers"
        tr = TraceCls(
            x=n,
            y=y,
            customdata=cd,
        ...
        if not enable_spline:
            tr.update(marker=dict(size=6, opacity=0.001, color=color))
...
def main():
    ...
    preselection_payload = build_preselection_payload_safe(
        data=data,
