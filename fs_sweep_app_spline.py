...
def main():
    ...
    preselection_cache_key = (
        f"preselection_payload:{data_id}:{seq_label}:{selected_location}:f{int(round(f_base))}"
    )
    raw_preselection_payload = st.session_state.get(preselection_cache_key)
    if not isinstance(raw_preselection_payload, dict):
        raw_preselection_payload = build_preselection_payload_safe(
            data=data,
    ...
    sel_bind_nonce_key = f"selection_bind_nonce:{data_id}:{chart_id}"
